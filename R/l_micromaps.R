...
l_micromaps <- function(tt = tktoplevel(), var_inspector = TRUE,
  ...
  # Input checks
  if (!is(tt, 'tkwin')) stop('tt should be generated by tktoplevel()')

  if (!is.logical(var_inspector) | length(var_inspector) > 1) stop('var_inspector should be TRUE or FALSE only')

  if (!is(spdf, 'SpatialPolygonsDataFrame')) stop('spdf must be of class SpatialPolygonsDataFrame')

  if (!is.character(labels)) stop('labels must be a vector of character strings')
  if (length(labels) != length(spdf)) stop('There should be 1 label for each element in spdf')

  if (!is.numeric(grouping) | (grouping %% 1 != 0) | length(grouping) > 1 | grouping <= 0) {
    stop('grouping must be a whole number')
  }

  if (grouping > 15) warning('A large value for grouping may make the micromap difficult to read')

  for (vars in c('ord.var', 'ord.var.axis', 'map.label', 'lab.label', 'title')) {
    if (!is.character(get(vars)) | length(get(vars)) > 1 | identical(get(vars), '')) stop(paste0(vars, ' must be a single non-empty string'))
  }

  if (!(ord.var %in% names(spdf@data))) stop('ord.var does not exist in spdf data')

  if (!is.null(ord.var.label) & (!is.character(ord.var.label) | length(ord.var.label) > 1 )) stop('Label for ord.var must be a single string or NULL')

  if (!is.null(var2)) {

    if (!is.character(var2) | length(var2) > 1) stop('var2 must be the name of the ordering variable')
    if (!(var2 %in% names(spdf@data))) stop('var2 does not exist in spdf data')

    if (!is.character(var2.axis) | length(var2.axis) > 1) stop('Axis label for var2 must be a single string')

    if (!is.null(var2.label) & (!is.character(var2.label) | length(var2.label) > 1 )) stop('Label for var2 must be a single string or NULL')
  ...
  delta_scale <- function(rng, zoom = 1, scale) {
  ...
  for (i in 1:n_groups) {
    ...
    l_configure(p_scat1[i],
                panX = rng1[1] * 0.98, zoomX = 1, deltaX = diff(rng1)*delta_scale(rng1, scale = 0.98),
    ...
    if (!is.null(var2)) {
      ...
      l_configure(p_scat2[i],
                  panX = rng2[1] * 0.98, zoomX = 1, deltaX = diff(rng2)*delta_scale(rng2, scale = 0.98),
                  panY = 0, zoomY = 1, deltaY = grouping + 1,
                  showLabels = T, xlabel = '', ylabel = '')
